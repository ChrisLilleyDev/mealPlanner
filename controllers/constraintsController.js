...
const getAllConstraints = asyncHandler(async (req, res) => {
    ...
    const constraints = await Constraint.find({ user }).lean().exec()
...
const createNewConstraint = asyncHandler(async (req, res) => {
    const { user } = req.userInfo
    const { name, ingredients, maxOccurance, shorttermAdj } = req.body

    // Confirm data
    if (!name || !Array.isArray(ingredients) || !ingredients.length || !maxOccurance || !shorttermAdj) {
    ...
    const duplicate = await Constraint.find({ user }).findOne({ name }).collation({ locale: 'en', strength: 2 }).lean().exec()
...
const updateConstraint = asyncHandler(async (req, res) => {
    const { user } = req.userInfo
    const { id, name, active, ingredients, maxOccurance, shorttermAdj } = req.body

    // Confirm data 
    if (!id || !name || typeof active !== 'boolean' || !Array.isArray(ingredients) || !ingredients.length || !maxOccurance || !shorttermAdj) {
        return res.status(400).json({ message: 'All fields are required' })
    }

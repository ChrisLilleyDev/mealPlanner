...
const getAllPlanTemplates = asyncHandler(async (req, res) => {
    ...
    const planTemplates = await PlanTemplate.find({ user }).lean().exec()
...
const createNewPlanTemplate = asyncHandler(async (req, res) => {
    const { user } = req.userInfo
    const { name, size, constraints } = req.body

    // Confirm data
    if (!name || !size || !Array.isArray(constraints) || !constraints.length) {
    ...
    const duplicate = await PlanTemplate.find({ user }).findOne({ name }).collation({ locale: 'en', strength: 2 }).lean().exec()
...
const updatePlanTemplate = asyncHandler(async (req, res) => {
    const { user } = req.userInfo
    const { id, name, active, size, constraints } = req.body

    // Confirm data 
    if (!id || !name || typeof active !== 'boolean' || !Array.isArray(constraints) || !constraints.length || !size) {
        return res.status(400).json({ message: 'All fields are required' })
    }
